...
export function convert_17(inputPaths, chosenPath, pointcloudName){
	...
	// ********************* added --start
	global.inputpaths.set(pointcloudName, inputPaths[0])
	...
	const extracting_time = () => {
		if (finish_flag==false) {
			let message = `转换中...`;
			viewer.postMessage(message, {duration: 3000}); 
			setTimeout(extracting_time, 3600); 
		}
	};
	extracting_time();   //计时执行时长
	// ********************* added by wzy --end
}
window.convert_17 = convert_17
...
export async function dropHandler(event){
	...
	for(let i = 0; i < event.dataTransfer.items.length; i++){
		...
		// let message = `Starting conversion.<br>
		// file number: ${event.dataTransfer.items.length}`;
		// viewer.postMessage(message, {duration: 15000});
